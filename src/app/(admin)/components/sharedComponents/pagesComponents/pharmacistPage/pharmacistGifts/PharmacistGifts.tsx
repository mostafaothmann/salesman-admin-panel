...
import { apiDoctor, apiPharmacist, apiSalesman, apiType } from "../../../../../../../stores/apis";
import { profileComponent } from "../../../../../../../stores/other-store-interfaces";


export default function PharmacistGifts({ profile_id }: profileComponent) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiPharmacist.get(`/gifts/${profile_id}`);
                setGifts(res.data);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
