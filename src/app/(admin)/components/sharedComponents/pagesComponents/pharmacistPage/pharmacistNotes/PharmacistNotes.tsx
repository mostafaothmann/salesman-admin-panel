...
import { Button, Checkbox, Image, Input, Modal, Skeleton, Space } from "antd";
...
export default function PharmacistNotes({ profile_id }: profileComponent) {
    const [visits, setVisits] = useState([])
    const { dataGovernorates,
        dataAreas } = usePlacesStore()
    const { getPharmacistVisitData, pharmacistVisitD } = useMedicalStore();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiPharmacist.get(`/visits/${profile_id}`);
                setVisits(res.data);
                const [
                    salesmanRes,
                    doctorRes,
    ...
    async function OpenShowModal(id: number) {
        setShownId(id);
        await getPharmacistVisitData(id);
                ...
                <div className="grid grid-cols-12 gap-2 col-span-12 sm:col-span-6 mt-5">
                            ...
                            value={
                                (() => {
                                    const doc = doctorsNames?.find(
                                        e => e.id === Number(pharmacistVisitD?.pharmacist_id)
                                    );
                            ...
                            value={
                                (() => {
                                    const a = assistantsNames?.find(
                                        e => e.id === Number(pharmacistVisitD?.assistant_id)
                                    );
                            ...
                            value={
                                (() => {
                                    const s = salesmansNames?.find(
                                        e => e.id === Number(pharmacistVisitD?.salesman_id)
                                    );
                    ...
                    <div className="col-span-12 sm:col-span-6">
                        ...
                        <Input
                            disabled
                            value={
                                typesNames?.find(e => e.id === Number(pharmacistVisitD?.type_id))
                                    ?.name ?? ""
                            }
                        />
                    </div>
                    ...
                    <div className="col-span-12 xl:col-span-6">
                        ...
                        <Input disabled value={pharmacistVisitD?.number_of_patients ?? ""} />
                    </div>

                    <div className="col-span-12 xl:col-span-6">
                        <h3 className="font-bold text-gray-700">أقرب الصيدليات:</h3>
                        <Input disabled value={pharmacistVisitD?.closest_pharmacy ?? ""} />
                    </div>

                    <div className="col-span-12 xl:col-span-6">
                        <h3 className="font-bold text-gray-700">تاريخ الزيارة:</h3>
                        <Input disabled value={pharmacistVisitD?.created_at?.slice?.(0, 10) ?? ""} />
                    </div>

                    <div className="col-span-12 xl:col-span-6">
                        <h3 className="font-bold text-gray-700">آخر المراجعة:</h3>
                        <Input disabled value={pharmacistVisitD?.validated_at?.slice?.(0, 10) ?? ""} />
                    </div>

                    <div className="col-span-12">
                        <h3 className="font-bold text-gray-700">ملاحظة المشرف:</h3>
                        <TextArea disabled value={pharmacistVisitD?.note} rows={4} />
                    </div>
