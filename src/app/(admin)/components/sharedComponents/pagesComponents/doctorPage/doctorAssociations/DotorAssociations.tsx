...
import { apiDoctor, apiAssociation } from "../../../../../../../stores/apis";


export default function DoctorAssociations({ profile_id }: profileComponent) {
    ...
    const { dataAssociations, associationD, getAssociationData, deleteAssociationDoctor, getAssociationsData, editAssociation, deleteAssociation, addAssociation, addAssociationDoctor } = useMedicalStore()
    ...
    const fetchDataAssociations = async () => {
        try {
            const res = await apiDoctor.get(`/associations/${profile_id}`);
    ...
    async function changeOpenModalAdd() {
        emptyFields();
        setOptionsAssociations(associationsNames?.map(e => { return { value: e.id, label: e.name } }))
        setOpen(true);
    }
    ...
    async function handleAdd() {
        ...
        try {
            const res = await addAssociationDoctor({
    ...
    async function handleDelete(id: number) {
        setLoading2(true);
        try {
            const res = await deleteAssociationDoctor(id);
            if (res?.status == 200) {
                notification.success({
                    message: "نجاح",
                    description: "تمت العملية بنجاح",
