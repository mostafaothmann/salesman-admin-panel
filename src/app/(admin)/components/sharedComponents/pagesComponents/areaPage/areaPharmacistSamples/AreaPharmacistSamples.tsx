...
import { apiDoctor, apiPharmacist, apiSalesman } from "../../../../../../../stores/apis";
import { profileComponent } from "../../../../../../../stores/other-store-interfaces";


export default function AreaPharmacistSamples({ profile_id }: profileComponent) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiPharmacist.get(`/samples/${profile_id}`);
                setSamples(res.data);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
