...
import { apiArea, apiDoctor, apiPharmacist, apiSalesman, apiType } from "../../../../../../../stores/apis";
import { profileComponent } from "../../../../../../../stores/other-store-interfaces";


export default function AreaPharmacistGifts({ profile_id }: profileComponent) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiArea.get(`/pharmacists-gifts/${profile_id}`);
                setGifts(res.data);
            } catch (error) {
                console.error("Error fetching data:", error);
            }
    ...
    const downloadExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "هدايا صيادلة المنطقة");
        XLSX.writeFile(workbook, "هدايا صيادلة المنطقة.xlsx");
    };
