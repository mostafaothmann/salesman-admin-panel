...
import { apiAssistant, apiPharmacist, apiType, apiSalesman, apiProduct, apiOffer, apiArea } from "../../../../../../../stores/apis";
...
export default function AreaOrders({ profile_id }: profileComponent) {
    const { deleteProduct, editOffer, editProduct, deleteOffer,
        getOrderData, orderD, editOrder,
    } = useCommercialStore();
    ...
    const fetchDataOrders = async () => {
        try {
            const res = await apiArea.get(`/orders/${profile_id}`);
            setOrders(res.data);
        } catch (error) {
    ...
    useEffect(() => {
        fetchDataOrders().finally(() => setPageLoading(false));
    }, [profile_id]);
