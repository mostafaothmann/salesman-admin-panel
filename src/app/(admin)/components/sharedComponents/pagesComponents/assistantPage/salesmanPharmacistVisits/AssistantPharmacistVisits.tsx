...
export default function AssistantPharmacistsVisits({ profile_id }: profileComponent) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiAssistant.get(`/pharmacist-visits/${profile_id}`);
                setVisits(res.data);
                const [
                    salesmanRes,
                    pharmacistRes,
                ...
                < div className="grid grid-cols-12 gap-2 col-span-12 sm:col-span-6 w-full h-full mt-5" >
                    ...
                    <div className="col-span-12 xl:col-span-3 md:col-span-3 sm:col-span-6" >
                        ...
                        < Input
                            disabled
                            value={
                                `${(pharmacistsNames?.find(e => e.id == Number(doctorVisitD?.doctor_id)))?.first_name} ${pharmacistsNames?.find(e => e.id == Number(doctorVisitD?.doctor_id))?.last_name}`
                            }
