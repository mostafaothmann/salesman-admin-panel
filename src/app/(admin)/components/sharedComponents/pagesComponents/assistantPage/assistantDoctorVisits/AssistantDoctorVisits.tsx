...
export default function AssistantDoctorVisits({ profile_id }: profileComponent) {
    const [visits, setVisits] = useState([])
    const { dataGovernorates, dataAreas } = usePlacesStore()
    const { getDoctorVisitData, doctorVisitD } = useMedicalStore();

    const [pageLoading, setPageLoading] = useState(true);
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiAssistant.get(`/doctor-visits/${profile_id}`);
                setVisits(res.data);
                const [
                    salesmanRes,
                    doctorRes,
