...
export default function TypeDoctorsVisits({ profile_id }: profileComponent) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiType.get(`/doctors-visits/${profile_id}`);
                setDoctorsVisits(res.data);
                const [
                    salesmanRes,
                    doctorRes,
                    typeRes,
    ...
    async function OpenLocationModal(id: number) {
        const doctorVisit = doctorsVisits?.find(e => e.id == id)
    ...
    //downloadExcele
    const downloadExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(doctorsVisits ?? []);
