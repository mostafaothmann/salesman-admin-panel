...
import { apiAssistant, apiPharmacist, apiType, apiSalesman, apiSample, apiBaseGift, apiGiftVisit } from "../../../../../../../stores/apis";
...
export default function TypePharmacistsVisits({ profile_id }: profileComponent) {
    const [pharmacistsVisits, setPharmacistsVisits] = useState([])
    ...
    const { getPharmacistVisitData, pharmacistVisitD, editPharmacistVisit } = useMedicalStore();
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await apiType.get(`/pharmacists-visits/${profile_id}`);
                setPharmacistsVisits(res.data);
    ...
    async function OpenLocationModal(id: number) {
        const pharmacistVisit = pharmacistsVisits?.find(e => e.id == id)
        setLan(Number(pharmacistVisit?.lan));
        setLat(Number(pharmacistVisit?.lat));
        setDoctorLan(Number(pharmacistVisit?.doctorLan));
        setDoctorLat(Number(pharmacistVisit?.doctorLat));
        // setLoading4(true);
    ...
    //downloadExcele
    const downloadExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(pharmacistsVisits ?? []);
