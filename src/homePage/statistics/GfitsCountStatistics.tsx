...
import { BaseGift, Gift, Video } from "../../icons";
import { useOtherStore } from "../../stores/otherStore/data.store";
import { apiGiftVisit } from "../../stores/apis";

export default function GiftsCountStatistics() {
    const [res, setRes] = useState([])
    useEffect(() => {
        const fetchData = async () => {
            try {
                const apiRes = await apiGiftVisit.get(`/all`);
                setRes(apiRes.data)
            } catch (error) {
