...
import { apiSample } from "../../stores/apis";

export default function SamplesCountStatistics() {
    const [res, setRes] = useState([])
    useEffect(() => {
        const fetchData = async () => {
            try {
                const apiRes = await apiSample.get(`/all`);
                setRes(apiRes.data)
            } catch (error) {
        ...
        <div className="w-full grid grid-cols-4 gap-4 sm:grid-cols-2 md:gap-6">
            ...
            <div className="w-full col-span-full rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                <div className="flex items-center justify-center w-12 h-12 bg-gray-100 rounded-xl dark:bg-gray-800">
                    <Sample className="text-gray-800 size-6 dark:text-white/90" />
                </div>
